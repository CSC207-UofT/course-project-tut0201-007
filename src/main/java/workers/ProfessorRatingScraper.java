...
public class ProfessorRatingScraper {
    ...
    public static double getRatingFromHTML(JsonObject json) throws RateMyProfessorException {
        String foundTeacher = json.getAsJsonObject("data").getAsJsonObject("search").getAsJsonObject("teachers").get("didFallback").toString();
        if (foundTeacher.equals("true")) {
            throw new RateMyProfessorException("Professor not found");
        }
        return json.getAsJsonObject("data").getAsJsonObject("search").getAsJsonObject("teachers").getAsJsonArray("edges").get(0).getAsJsonObject().getAsJsonObject("node").get("avgRating").getAsDouble();
    }
